...
export function useSocket(): UseSocketReturn {
  ...
  useEffect(() => {
    ...
    socket.on("ap_event_sent", (data: ApEventSent) => {
      setApEvents((prev) => [...prev.slice(-49), data]);
      setApStatus((prev) => ({
        ...prev,
        sent: data.success ? prev.sent + 1 : prev.sent,
        failed: data.success ? prev.failed : prev.failed + 1,
        lastSentAt: data.success ? data.timestamp : prev.lastSentAt,
        lastError: data.error || null,
      }));
      if (data.success) {
        addLog(`AP: ${data.eventType} forwarded`, "activepieces");
      } else {
        addLog(`AP: ${data.eventType} failed - ${data.error}`, "error");
      }
    });
    ...
    socket.on("debug_event", (evt: DebugEvent) => {
      const callEvents = [
        "newchannel", "dialbegin", "dialend", "newstate", "hangup", "newexten",
      ];
